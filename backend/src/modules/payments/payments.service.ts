...
export async function list(req: Request) { return prisma.$queryRawUnsafe("select p.*, u.name as paid_by_admin_name from payments p left join users u on u.id = p.paid_by_admin_id where ($1::uuid is null or p.branch_id=$1::uuid) order by p.created_at desc", req.user?.role === "admin" ? req.user.branch_id : req.query.branch_id === "all" ? null : req.query.branch_id ?? null); }
