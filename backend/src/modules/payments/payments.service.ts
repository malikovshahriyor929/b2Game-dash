...
export async function list(req: Request) { return prisma.$queryRawUnsafe("select p.*, u.name as paid_by_admin_name from payments p left join users u on u.id = p.paid_by_admin_id where ($1::uuid is null or p.branch_id=$1::uuid) order by p.created_at desc", baseRole(req.user?.role) === "admin" ? (req.user?.branch_id ?? null) : req.query.branch_id === "all" ? null : req.query.branch_id ?? null); }
export async function create(req: Request) {
  ...
  const rows = await prisma.$queryRawUnsafe<any[]>("insert into payments(branch_id,shift_id,session_id,sale_id,customer_id,amount,method,cash_amount,card_amount,qr_amount,balance_amount,received_amount,change_amount,paid_by_admin_id) values($1::uuid,$2::uuid,$3::uuid,$4::uuid,$5::uuid,$6,$7,$8,$9,$10,$11,$12,$13,$14::uuid) returning *", branchId, shiftId, req.body.session_id ?? null, req.body.sale_id ?? null, req.body.customer_id ?? null, total, req.body.method, req.body.cash_amount, req.body.card_amount, req.body.qr_amount, req.body.balance_amount, receivedAmount, changeAmount, req.user!.user_id);
