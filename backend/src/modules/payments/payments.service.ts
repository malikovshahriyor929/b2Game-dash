...
export async function create(req: Request) {
  ...
  const rows = await prisma.$queryRawUnsafe<any[]>("insert into payments(branch_id,session_id,sale_id,customer_id,amount,method,cash_amount,card_amount,qr_amount,balance_amount,paid_by_admin_id) values($1::uuid,$2::uuid,$3::uuid,$4::uuid,$5,$6,$7,$8,$9,$10,$11::uuid) returning *", branchId, req.body.session_id ?? null, req.body.sale_id ?? null, req.body.customer_id ?? null, total, req.body.method, req.body.cash_amount, req.body.card_amount, req.body.qr_amount, req.body.balance_amount, req.user!.user_id);
  ...
  await auditLog({ actor: req.user, branch_id: branchId, action_type: "payment_created", entity_type: "payment", entity_id: rows[0].id, session_id: req.body.session_id ?? null, amount: total });
