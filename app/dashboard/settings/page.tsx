...
export default function SettingsPage() {
  return (
    <div>
      ...
      <Tabs defaultValue="general"><TabsList className="max-w-full overflow-x-auto"><TabsTrigger value="general">General</TabsTrigger><TabsTrigger value="roles">Roles</TabsTrigger><TabsTrigger value="shift">Shift flow</TabsTrigger><TabsTrigger value="devices">Devices</TabsTrigger></TabsList>
        <TabsContent value="general"><div className="grid grid-cols-[repeat(auto-fit,minmax(180px,1fr))] gap-3">{sections.map(([label, Icon]) => <Card key={label} className="p-4"><Icon className="mb-3 text-2xl text-sky-300" /><div className="font-semibold">{label}</div><div className="mt-1 text-sm text-slate-500">Mock configuration panel</div></Card>)}</div></TabsContent>
